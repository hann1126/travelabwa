...
<body class="font-poppins text-black">
  <section id="content" class="max-w-[640px] w-full mx-auto bg-[#F9F2EF] min-h-screen flex flex-col gap-8 pb-[120px]">
    ...
    <div class="flex flex-col gap-3 px-4">
      ...
      <a href="{{ route('dashboard.bookings_details', $booking->id) }}" class="card">
      <div class="bg-white p-4 rounded-[26px] flex items-center gap-4">
        ...
        <div class="w-[92px] h-[92px] flex shrink-0 rounded-xl overflow-hidden">
          <img src="{{ $booking->tour ? Storage::url($booking->tour->thumbnail) : asset('default-thumbnail.jpg') }}"
          class="w-full h-full object-cover object-center" alt="thumbnail">

        </div>
        <div class="flex flex-col gap-1">
          <p class="font-semibold text-sm tracking-035 leading-[22px]">
          {{ optional($booking->tour)->name ?? 'Tour not found' }}
          </p>
          <p class="font-semibold text-sm tracking-035 leading-[22px]">
          {{ optional($booking->tour)->name ?? 'Tour not found' }}
          </p>
          ...
          {{ optional($booking->tour)->day ?? 'N/A' }} days |
          {{ $booking->quantity }} packs
          </p>
    ...
    @empty
      <p>Anda belum pernah melakukan boking</p>
    @endforelse
    </div>
    <div
