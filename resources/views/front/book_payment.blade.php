...
<section id="content" class="max-w-[640px] w-full mx-auto bg-[#F9F2EF] min-h-screen flex flex-col gap-8 pb-[120px]">
    ...
    <form enctype="multipart/form-data" action="{{ route('front.book_payment_store', $packageBooking->id) }}" method="POST" class="flex flex-col gap-8">
        @csrf
        @method('PATCH')
            ...
            <div class="bg-white p-4 rounded-[26px] flex items-center gap-3">
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <div class="flex gap-1 items-center">
                        <div class="w-4 h-4">
                            <img src="{{ asset('assets/icons/calendar-grey.svg') }}" alt="icon">
                        </div>
                        <span class="text-darkGrey text-sm tracking-035 leading-[22px]">
                            {{ \Carbon\Carbon::parse($packageBooking->start_date)->format('M d, Y') }} - {{ \Carbon\Carbon::parse($packageBooking->end_date)->format('M d, Y') }}
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white p-[16px_24px] rounded-[26px] flex flex-col gap-4">
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <span id="bank-number" class="text-sm tracking-035 leading-[22px]">{{ $packageBooking->bank->bank_account_number }}</span>
                        <button type="button" class="font-semibold text-sm tracking-035 leading-[22px] text-blue w-fit ml-auto" onclick="copyText('bank-number')">Copy</button>
                    </div>
                </div>
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <div class="flex items-center justify-between">
                        <span id="total-payment" class="font-semibold text-lg tracking-[0.6px] leading-[26px]">Rp {{ number_format($packageBooking->total_amount, 0, ',', '.') }}</span>
                        <button type="button" class="font-semibold text-sm tracking-035 leading-[22px] text-blue w-fit ml-auto" onclick="copyText('total-payment')">Copy</button>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="flex items-center gap-3">
                <button type="button" id="upload-file" class="bg-white border border-[#BFBFBF] p-[16px_0_16px_12px] rounded-[10px] overflow-hidden w-full">
                    ...
                    <div id="file-info" class="hidden flex flex-row flex-nowrap gap-3 items-center">
                        ...
                        <span id="fileName" class="text-sm text-nowrap">Nama File</span>
                    </div>
                </button>
                <input type="file" name="proof" id="file" class="hidden" accept="image/*,.pdf">
            </div>
